...
public class HashTableQuad {
    ...
    public void insert(int n) {
        ...
        while(filled[key]) {    // if collision...
            ...
            key = (oldKey + i*i) % length;    // quadratically probe to new index
            i++;
            probes++;
        }
    ...
    private void rehash() {
        ...
        for(int i=0; i<length; i++) {
            if(filled[i]) {
                ...
                int j=1;                            // offset for quadratic probe
                while(newFilled[key]) {         // while there are collisions...
                    key = (oldKey + j*j) % newLength; // ...quadratically probe
                    j++;
                }
        ...
        maxNum = (double)(int)temp == temp ? (int)temp : (int)(temp+1);
    }
